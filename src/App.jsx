...
function App() {
  ...
  const playagain = () => {
    ...
    setCompleted(false);
    setTimeLeft(15);
    setGameStarted(false); // Go back to home after play again? No, let's just restart quiz.
    // Actually, user said "Home page saying newgame", so maybe play again should go back to home?
    // Or just restart. Let's make playagain just restart the quiz, and we can add a "Home" button later if needed.
    // But for now, let's just make the initial entry go to home.
  }
              ...
              >
                ...
                <div className="flex gap-6 justify-center mb-8">
                  <div className="bg-white/5 p-4 rounded-2xl border border-white/5 flex-1">
                    <p className="text-emerald-400 text-xs font-bold uppercase tracking-wider mb-1">Right Answer</p>
                    <p className="text-2xl font-bold text-white">{baseScore}</p>
                  </div>
                  <div className="bg-white/5 p-4 rounded-2xl border border-white/5 flex-1">
                    <p className="text-sky-400 text-xs font-bold uppercase tracking-wider mb-1">Time Bonus</p>
